...
def _beis_counter_file() -> str:
    return os.path.join(_counter_dir(), "beis_export_counter.txt")


def _read_beis_last_assigned() -> int:
    try:
        with open(_beis_counter_file(), "r", encoding="utf-8") as fh:
            return max(0, int(fh.read().strip() or "0"))
    except (OSError, ValueError):
        return 0


def _write_beis_last_assigned(n: int) -> None:
    path = _beis_counter_file()
    tmp = path + ".tmp"
    with open(tmp, "w", encoding="utf-8") as fh:
        fh.write(str(n))
        fh.flush()
...
def peek_next_beis_batch_number() -> int:
    return _read_beis_last_assigned() + 1


def commit_beis_batch_number(n: int) -> None:
    _write_beis_last_assigned(max(_read_beis_last_assigned(), n))
